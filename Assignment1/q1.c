...
int data(frac *fracs)
{
  ...
  printf("Enter fractions by pairs of integers; Enter 0 0 to finish.\n");
    ...
    scanf("%d %d", &fracs[counter].numerator, &fracs[counter].denominator);
		if ((fracs[counter].denominator == 0) && (fracs[counter].numerator != 0)) //Logic test, can't divide by zero
    {
      do
      {
      ...
      while (fracs[counter].denominator == 0); //As long as the condition is true
		}
    assert(fracs = (frac*)realloc(fracs, (counter+1)*sizeof(frac)));
    counter++;
	}
	while(fracs[counter-1].numerator != 0 && fracs[counter-1].denominator != 0);
  return (counter-1); //Sending back the amount of fractions
}
